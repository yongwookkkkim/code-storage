...
v1=np.array([9,8,7,6])
v2=np.array([1,2,3,4])
print(np.vstack([v1,v2])) #horizontal stack (very similar) exists

#loading data from a txt file
filedata=np.genfromtxt('filename.txt', delimiter=', ') 
filedata=filedata.astype('int32') #filedata.astype() alone only makes a copy

#advanced indexing and boolean masking
print(filedata>50) -> gives a matrix of same dimension, each item of the value (filedata>50)
filedata[filedata>50] -> only the values above 50 are displayed

#you can index with a list in numpy
a=np.array([1,2,3,4,5,6,7,8])
print(a[[1,2,8]])

np.any(filedata>50, axis=0) #whether any of the item each column are above 50
np.all(filedata>50, axis=0) #whether all of the item each column are above 50
